...
public class JSpinner4j extends JSpinner
{
	...
	private void applyFocusHighlight()
	{
		SwingUtilities.invokeLater(() -> {
			...
			if (editor instanceof DefaultEditor)
			{
				...
				textField.setFont(ZPLCommon.font_std);
				...
				textField.addFocusListener(new FocusAdapter()
				{
					...
					{
						textField.setBackground(ZPLCommon.color_textfield_background_focus_color);
					}

					@Override
					public void focusLost(FocusEvent e)
					{
						textField.setBackground(ZPLCommon.color_textfield_background_nofocus_color);
					}
				});
			}
		});
	...
	private void applyStyle()
	{
		...
		tf.setDisabledTextColor(ZPLCommon.color_textfield_foreground_disabled);

		if (!isEnabled())
		{
			tf.setBackground(ZPLCommon.color_textfield_background_disabled);
			tf.setForeground(ZPLCommon.color_textfield_foreground_disabled);
		}
		else if (!isEditable())
		{
			tf.setBackground(ZPLCommon.color_textfield_background_disabled);
		...
		{
			tf.setBackground(ZPLCommon.color_textfield_background_nofocus_color);
			tf.setForeground(ZPLCommon.color_textfield_forground_nofocus_color);
		}
	}
