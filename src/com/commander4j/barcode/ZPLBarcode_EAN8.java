...
public class ZPLBarcode_EAN8
{
	...
	public boolean getParameters(ZPLCmd cmd,ZPLMemory memory)
	{
		...
		memory.bps.store("^BY", ZPLPropertyStore.Param_Barcode_Type, ZPLBarcode_Types.B8_EAN8);
				
		memory.bps.store("^B8",ZPLPropertyStore.Param_Orientation,util.getStringWithDefault(cmd, 0, memory.bps.recallAsStringWithDefault("^FW",ZPLPropertyStore.Param_Orientation, "N")));	
		
		memory.bps.store("^B8",ZPLPropertyStore.Param_Barcode_Height,util.getIntWithDefault(cmd, 1, memory.bps.recallAsIntegerWithDefault("^BY",ZPLPropertyStore.Param_Barcode_Height, 100)));	

		memory.bps.store("^B8",ZPLPropertyStore.Param_Barcode_Interpretation,(util.getStringWithDefault(cmd, 2, "Y")));	
		
		memory.bps.store("^B8",ZPLPropertyStore.Param_Barcode_Interpretation_Above,(util.getStringWithDefault(cmd, 3, "N")));	
	...
	public boolean create(Graphics g,float magnification,ZPLMemory memory)
	{
		...
		bean.setMode(Mode.EAN8);
		
		bean.setModuleWidth((int) memory.bps.recallAsDoubleWithDefault("^BY",ZPLPropertyStore.Param_Module_Width,0.2));
		
		bean.setBarHeight((int) (memory.bps.recallAsDoubleWithDefault("^B8",ZPLPropertyStore.Param_Barcode_Height,100)));
		...
		if (memory.bps.recallAsStringWithDefault("^B8",ZPLPropertyStore.Param_Barcode_Interpretation,"N").equals("Y"))
		{
			...
			if (memory.bps.recallAsStringWithDefault("^B8",ZPLPropertyStore.Param_Barcode_Interpretation_Above,"N").equals("Y"))
			{
