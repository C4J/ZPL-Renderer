...
public class ZPLBarcode_Code128
{
	...
	public boolean getParameters(ZPLCmd cmd,ZPLMemory memory)
	{
		...
		memory.bps.store("^BY", ZPLPropertyStore.Param_Barcode_Type, ZPLBarcode_Types.BC_Code128);

		memory.bps.store("^BC",ZPLPropertyStore.Param_Orientation,(util.getStringWithDefault(cmd, 0, "N")));

		memory.bps.store("^BC",ZPLPropertyStore.Param_Barcode_Height,util.getIntWithDefault(cmd, 1, memory.bps.recallAsIntegerWithDefault("^BY",ZPLPropertyStore.Param_Barcode_Height, 100)));	

		memory.bps.store("^BC",ZPLPropertyStore.Param_Barcode_Interpretation,(util.getStringWithDefault(cmd, 2, "Y")));	
		
		memory.bps.store("^BC",ZPLPropertyStore.Param_Barcode_Interpretation_Above,(util.getStringWithDefault(cmd, 3, "N")));	

		memory.bps.store("^BC",ZPLPropertyStore.Param_Barcode_UCC_Check_Digit,(util.getStringWithDefault(cmd, 4, "N")));	
		
		memory.bps.store("^BC",ZPLPropertyStore.Param_Barcode_Code128_Mode,util.getStringWithDefault(cmd, 5, "N"));	
	...
	public boolean create(Graphics g,float magnification,ZPLMemory memory)
	{
		...
		Code128 bean = new Code128();
		
		bean.setModuleWidth((int) memory.bps.recallAsDoubleWithDefault("^BY",ZPLPropertyStore.Param_Module_Width,2));
		
		bean.setBarHeight((int) (memory.bps.recallAsDoubleWithDefault("^BC",ZPLPropertyStore.Param_Barcode_Height,100)));
		...
		{	
			...
			if (memory.bps.recallAsStringWithDefault("^BC",ZPLPropertyStore.Param_Barcode_Interpretation_Above,"N").equals("Y"))
			{
