...
public class ZPLBarcode_EAN13
{
	...
	public boolean getParameters(ZPLCmd cmd,ZPLMemory memory)
	{
		...
		memory.bps.store("^BY", ZPLPropertyStore.Param_Barcode_Type, ZPLBarcode_Types.BE_EAN13);
				
		memory.bps.store("^BE",ZPLPropertyStore.Param_Orientation,util.getStringWithDefault(cmd, 0, memory.bps.recallAsStringWithDefault("^FW",ZPLPropertyStore.Param_Orientation, "N")));	
		
		memory.bps.store("^BE",ZPLPropertyStore.Param_Barcode_Height,util.getIntWithDefault(cmd, 1, memory.bps.recallAsIntegerWithDefault("^BY",ZPLPropertyStore.Param_Barcode_Height, 100)));	

		memory.bps.store("^BE",ZPLPropertyStore.Param_Barcode_Interpretation,(util.getStringWithDefault(cmd, 2, "Y")));	
		
		memory.bps.store("^BE",ZPLPropertyStore.Param_Barcode_Interpretation_Above,(util.getStringWithDefault(cmd, 3, "N")));	
	...
	public boolean create(Graphics g,float magnification,ZPLMemory memory)
	{
		...
		bean.setMode(Mode.EAN13);
		
		bean.setModuleWidth((int) memory.bps.recallAsDoubleWithDefault("^BY",ZPLPropertyStore.Param_Module_Width,0.2));
		
		bean.setBarHeight((int) (memory.bps.recallAsDoubleWithDefault("^BE",ZPLPropertyStore.Param_Barcode_Height,100)));
		...
		if (memory.bps.recallAsStringWithDefault("^BE",ZPLPropertyStore.Param_Barcode_Interpretation,"N").equals("Y"))
		{
			ZPLFontCache ch = memory.zplFont.recallFont(g2d, new ZPLCmd(""), memory, magnification,true);
			
			bean.setFont(ch.font);
			
			if (memory.bps.recallAsStringWithDefault("^BE",ZPLPropertyStore.Param_Barcode_Interpretation_Above,"N").equals("Y"))
			{
