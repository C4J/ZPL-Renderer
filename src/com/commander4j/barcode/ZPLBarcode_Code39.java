...
public class ZPLBarcode_Code39
{
	...
	public boolean getParameters(ZPLCmd cmd,ZPLMemory memory)
	{
		...
		memory.bps.store("^BY",ZPLPropertyStore.Param_Barcode_Type, ZPLBarcode_Types.B3_Code39);
				
		memory.bps.store("^B3",ZPLPropertyStore.Param_Orientation,util.getStringWithDefault(cmd, 0, memory.bps.recallAsStringWithDefault("^FW",ZPLPropertyStore.Param_Orientation, "N")));	

		memory.bps.store("^B3",ZPLPropertyStore.Param_Mod43_CheckDigit,(util.getStringWithDefault(cmd, 1, "N")));
		
		memory.bps.store("^B3",ZPLPropertyStore.Param_Barcode_Height,util.getIntWithDefault(cmd, 2, memory.bps.recallAsIntegerWithDefault("^BY",ZPLPropertyStore.Param_Barcode_Height, 100)));	

		memory.bps.store("^B3",ZPLPropertyStore.Param_Barcode_Interpretation,(util.getStringWithDefault(cmd, 3, "Y")));	
		
		memory.bps.store("^B3",ZPLPropertyStore.Param_Barcode_Interpretation_Above,(util.getStringWithDefault(cmd, 4, "N")));	
	...
	public boolean create(Graphics g,float magnification,ZPLMemory memory)
	{
		...
		Code3Of9 bean = new Code3Of9();
		
		bean.setModuleWidth((int) memory.bps.recallAsDoubleWithDefault("^BY",ZPLPropertyStore.Param_Module_Width,0.2));
		
		bean.setBarHeight((int) (memory.bps.recallAsDoubleWithDefault("^B3",ZPLPropertyStore.Param_Barcode_Height,100)));
		
		if (memory.bps.recallAsStringWithDefault("^B3", ZPLPropertyStore.Param_Mod43_CheckDigit, "N").equals("Y"))
		{
			bean.setCheckDigit(CheckDigit.MOD43);
		}
		else
		{
			bean.setCheckDigit(CheckDigit.NONE);
		}
		...
		{	
			...
			if (memory.bps.recallAsStringWithDefault("^B3",ZPLPropertyStore.Param_Barcode_Interpretation_Above,"N").equals("Y"))
			{
